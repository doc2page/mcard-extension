...
🎯 定向搜索（词条筛选）· 💰 手动买/卖/撤 · ♻️ 兑换机制符（10 换 1）· 📦 批量买/卖/撤/改价 · 🚫 自挂单防自买 · 🔴 变化角标 · 💎 魔力预算池 · 📊 交易挂单 · 📈 掉落统计（+📥 手动导入补全）· 💹 市场数据（+📄 分析报告）· 📚 我的卡册 · 👤 用户画像 · 👁 隐私遮罩 · 🌍 双站点(.cc/.io) · 🎨 深浅主题 · 🌐 中英双语
...
直连 [M-Team 卡牌市场](https://kp.m-team.cc/cards/market) API 接口采集市场卡片、机制卡、交易记录、当前挂单、持有卡片、个人资料与魔力（Bonus）明细——**全部由你手动触发**（点刷新 / 切视图 / 买·卖·撤），无后台轮询、无定时器、无推送通道。支持**手动买/卖/撤**（限价单语义 + 魔力预算池控额 + 自挂单防自买）、**定向搜索**（词条筛选：稀有度/称号/每档最大价）、**普通卡兑换机制符**（10 换 1，自动选最便宜）、**侧栏变化角标**（轻量探测差值，进视图清零）、**交易记录 / 当前挂单 / 持有卡片**管理、**掉落概率统计**（feed 增量 + tab 全量双源采集，支持手动导入补全历史）、**我的卡册**（持有+挂单按影片聚合稀有度/称号矩阵）、**用户画像**（综合余额·产能·交易·掉落·符券博弈，刻财富等级/消费趋势/五维雷达）、**市场数据**（全站成交历史聚合 + 一键生成市场分析报告）、中英双语界面与深浅色主题。
...
3. **市场筛选**（市场 view 顶部筛选条）：初始安装默认**全选所有稀有度与机制卡**（首刷即全量采集）；勾选要查看的稀有度 / 机制卡 → 为每个分类设定 lowestAsk 阈值（留空=不限，点「价格阈值」弹模态设置，含预设方案保存/加载，改阈值即时生效）→ 卡片网格显示符合各分类阈值的低价卡（含海报、价格、涨跌），点击卡片跳转详情页；筛选条可按**称号**多选筛选，卡片区右上角切换按价格/按分类 + **刷新按钮**（所有刷新路径统一入口：8s 节流防连点，按钮旋转与请求生命周期同步；批量买入/保存令牌后自动绕节流强刷）
4. **定向搜索（词条筛选）**：市场 view 顶部「定向搜索」行点 `+` 弹词条模态——片名 + 稀有度（勾选 + 每档最大价）+ 称号（多选，不选=全要），按 `/api/pt-card/market/search` 逐词条查询、前端过滤（勾称号时无称号卡排除）合并去重；胶囊按词条名稳定着色、✎ 随时编辑；卡册放大镜联动（已设词条变铅笔）；有 tag 时市场 view 切到搜索结果并隐藏稀有度/机制卡/称号/价格阈值筛选，删空 tag 回到常规市场视图
5. **手动购买**：卡片上的购买按钮走 `/api/pt-card/market/buy` 直连（限价单语义，见「手动购买与限价单语义」），从预算魔力池扣额
6. **持有卡卖出 / 挂单管理**：见「持有卡卖出与挂单管理」
7. **交易记录 / 当前挂单 / 持有卡片 / 掉落统计 / 用户画像 / 市场数据**：侧栏入口拉取成交、挂单、持有数据（含统计面板与多维筛选，卖出价按扣 5% 手续费后的净收入显示（成交价 ÷ 1.05，悬浮可查成交价），卡片可点**询价**按钮查最高买价，交易按 tradedAt、挂单/持有按最后更新时间（lastModifiedDate）排序，搜索区可切换升降序）；「掉落统计」点视图触发：日常走 `/pt-card/feed` 直连增量刷新（最新 25 条，msg 最新之后），距上次刷新 >7 天则开后台 tab 全量重采（msg/search 翻页，**翻到掉卡活动起点（2026-07-01）之前即视为完整收齐**，不必翻完所有页），tab 拿不全（如浏览器未登录 M-Team）时 hero 区提示「当前只获取到 N 条记录」并可**手动导入**（自己到 message 页搜「卡片掉落」复制响应 JSON 粘贴，支持多页分批导入）；展示历史掉落概率（稀有度/称号横向分布）、每日掉落柱状图（稀有度堆叠，7/30/90/全部可切换）与活跃度（总历时/掉落天数/最大连续/日均），并附**魔力符券使用记录**卡片（开卡数量 / 总收益 / 平均（推荐购买价）/ 中位（盈亏线）/ 单卡最高 / 最低 / 幸运倍率 / 收益波动 + 收益分布横向条形 + 历史开卡面积折线）；「**我的卡册**」持有 + 在挂按影片聚合（排除机制卡）：每部影片一行反色卡，5 稀有度 × 5 称号双格矩阵（左持有墨绿 / 右挂单金，暗淡层级一眼看出缺口），按稀有度种数 → 档位加权和 → 称号含金量排序，右上角放大镜一键把片名加入定向搜索（已设词条变铅笔，点击编辑筛选条件）；「用户画像」每日采集 Bonus（每小时魔力产能 finalBs，显示在资料卡魔力右下角如 `94/h`），综合余额/产能/交易收支/掉落运气/符券博弈，给出财富等级（贫穷→老钱 9 档）、消费趋势（积攒/自给/消耗/透支）与五维雷达（财力·产能·消费·运气·**博弈**——基于魔力符券开卡的赌性/欧皇度/波动，判定博弈人格未涉足/尝鲜/欧皇赌徒/过山车/苦挖并带入称号）；运气构成面板附**符券博弈摘要**（开卡数/总收益/平均/最高/最低/幸运倍率）；「**市场数据**」聚合全站成交历史（首次保存令牌后后台全量、点按钮增量），看大盘 KPI + 时间走势 + 卡视角/人视角排行，并可**一键生成市场分析报告**（详见「市场数据」段）
...
手动购买走 `mtFetch` 直连 `/api/pt-card/market/buy`（body：filmId/rarity/provenance/price），**price = 入库的 lowestAsk**（即你点击购买时卡片的当前最低叫价）。买前仅校验 `入库价 ≤ 设定阈值`（硬上限）与预算池余额；**购买流程单飞锁**（确认弹窗到提交全程只允许一个，防连点双买）。
...
成交后扣减预算池、刷新该分类；交易记录仍由「交易记录」视图增量同步。失败提示（卖单不存在/已提价、请求失败、超阈值、预算不足等）以 toast 呈现并附行动指引（余额不足→去充值、预算不足→提高总额或重置、令牌无效→检查复制或重新生成）。

**自挂单防自买**：市场里同款（filmId+rarity+provenance）同价 = 你自己的挂单（最低价就是你在卖）——单卡购买按钮灰显置「这是你自己的挂单」，批量模式禁止勾选（点选被拦 + 勾选框隐藏），杜绝买自己。
...
- **当前挂单管理**：当前挂单视图每张挂单有「修改」按钮，弹窗二选一——**取消挂单**（`/cancel`）或**修改挂单价**（官方无改价接口：挂卖单先 `cancel` 再用新价 `sell` 重新挂出；**挂买单**先撤再按新价**限价重挂**（未成交即挂买单，市场已有更低价则直接成交）；确认按钮 loading 至网络操作完成，期间 Esc/取消全拦）；搜索条「挂买 / 挂卖」互斥单选筛选（再点取消）；撤单/改价全程单飞锁，连点不重复。
- **锁定卡**：交易锁（平台临时，到期自动解）显示「冷却中」（冷却卡**可参与兑换**，官方允许）；**手动锁**（你主动锁防误卖，跨会话持久）默认**隐藏**，筛选条「显示锁定」勾选后可见，解锁两步点击确认。
- **普通卡兑换机制符（10 换 1）**：持有视图统计卡显示 3 配方可兑数量（1=魔力符N / 2=置顶免费符SR / 3=VIP符UR，可兑数=对应稀有度未锁定卡÷10）与已兑换历史；点小卡进**兑换子模式**（列表锁定该稀有度、选满 10 张封顶）——**自动选**按称号升序取最便宜的前 10（无称号→傳火），确认模态 10 卡清单二次确认 + 前后端双重校验，成功 toast 符名并刷新持有。
- **侧栏变化角标**：打开面板轻量探测各数据源差值（5 个小请求只拿 total，不落盘），侧栏按钮红底白字胶囊显示新增数（99+ 封顶），点进对应视图即清零。
...
- **市场 · 批量购买**：选中多张 → 入口即前置校验预算（未设池 / 可用额度小于所选总价直接 toast 区分原因与金额明细，不开模态）→ 各自最低叫价（lowestAsk）串行买入，提交前再校验总花费 vs 可用预算；限价单语义保证不超价，盘口变动未成交显示灰色「未成交」（不进重试集）；自挂单不可选。
- **持有 · 批量卖出**：选中多张 → 顶部按稀有度/称号批量设净价（或逐项填）→ 串行上架；卖出新增挂单，校验 50 挂单上限；价格输入框**双态**——聚焦显示净卖价可编辑、失焦显示挂单价（净×1.05，弱化 + 悬浮双价对照）。
- **挂单 · 批量取消**：选中多单 → 串行撤单。
- **挂单 · 批量改价**：选中多单 → 设新净价 → 每单先撤再用新价重挂（挂卖 cancel + sell 两步；挂买单 cancel + 限价重挂）。
...
**市场分析报告**：tab 条尾部「生成分析报告」按钮，对全量历史（**排除当天**，基准日=昨天）一键生成打印级 HTML 研报，新 tab 打开：① 量价诊断四象限（扩张/出货/背离/降温/横盘）② 主导结构（稀有度 GMV 占比 + 机制卡占比 + 集中度 HHI，普通卡与机制卡统一计入总 GMV）③ 流动性成色（资金净流入/流出、纯买纯卖占比、倒卖毛利与持有天数中位数）④ 定价合理性（偏态 + 稀有度价格阶梯 + 离散度 + 价格分布直方图）⑤ 热门成交，顶部一句话摘要。判读阈值语言无关（tier key），报告随界面语言（中/英）生成，可直接打印/存 PDF。
...
- 本地存储键：`config`（市场筛选/阈值/预设/预算/语言/定向搜索设置）、`mtApiKey`（API 直连令牌，独立 key，不进 config）、`buckets`/`mechBucket`（各分类最新市场结果）、`history`（采集事件日志）、`buyHistory`（交易记录）、`ordersAll`（挂单）、`inventory`/`mechInventory`（持有卡片/机制卡）、`lockedCards`（手动锁）、`redemptions`（兑换机制符历史）、`profile`（账户资料）、`stats`（采集统计）、`dropStats`（掉落统计：原始消息表 + 概率/活跃度聚合）、`bonus`（用户 Bonus：finalBs 每小时产能 + 全量明细）、`cancelFailedOrders`（撤单重试全失败的残留挂单记录）。

## 配置

- **市场筛选**：稀有度（UR/SSR/SR/R/N）、机制卡（魔力符券/置顶免费符/VIP七日符）、每分类价格阈值（lowestAsk ≤，留空=不限）、阈值预设方案（命名保存/加载/删除）
- **定向搜索**：词条列表 `{片名, 稀有度[], 称号[], 每档最大价{}}`（旧纯字符串词条兼容=无筛选；逐词条串行查询、前端过滤、合并去重）
- **预算魔力池**：总额、重置
