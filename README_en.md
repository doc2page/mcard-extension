...
🎯 Directed search (tag filters) · 💰 Manual buy/sell/cancel · ♻️ Redeem mech cards (10-for-1) · 📦 Batch buy/sell/cancel/reprice · 🚫 Own-listing guard · 🔴 Change badges · 💎 Bonus budget pool · 📊 Trades & orders · 📈 Drop stats (+📥 manual import backfill) · 💹 Market data (+📄 analysis report) · 📚 Card binder · 👤 Player portrait · 👁 Privacy mask · 🌍 Dual-site (.cc/.io) · 🎨 Dark/Light · 🌐 EN/中文
...
Talks directly to the [M-Team card market](https://kp.m-team.cc/cards/market) API to collect market cards, mech cards, trade history, open orders, holdings, profile and Bonus details — **all triggered manually by you** (click refresh / switch views / buy·sell·cancel), with no background polling, no timer, no push channel. Supports **manual buy/sell/cancel** (limit-order semantics + bonus budget cap + own-listing guard), **directed search** (tag filters: rarity/title/per-rarity max price), **normal-card redemption for mech cards** (10-for-1, auto-pick cheapest), **sidebar change badges** (lightweight total probe, cleared on view entry), **trade history / open orders / holdings** management, **drop probability stats** (dual-source collection: feed incremental + background-tab full backfill, with manual JSON import to backfill history), **card binder** (holdings + listings aggregated by film into a rarity/title matrix), **player portrait** (wealth tier / spending trend / 5-dim radar synthesized from balance · income · trades · drops · voucher gambling), **market data** (full-site trade-history aggregation + one-click market analysis report), bilingual UI and dark/light themes.
...
3. **Market filter** (market-view filter bar at top): on fresh install **all rarities and mech cards are checked by default** (first refresh collects everything); check the rarities / mech cards to view → set a lowestAsk threshold per category (blank = unlimited; click "Price threshold" for a modal with preset save/load, changes apply instantly); the card grid shows low-price cards matching each category's threshold (poster, price, delta); click a card to open its detail page; filter by **title** from the filter bar (multi-select), toggle sort by price / by category at the top-right of the card area + the **refresh button** (unified entry for all refresh paths: 8s throttle, spinner synced to the request lifecycle; batch buys / token saves force-refresh past the throttle)
4. **Directed search (tag filters)**: tap `+` on the "Directed search" row for a tag modal — film title + rarities (checkboxes, per-rarity max price) + titles (multi-select, none = all); each tag queries `/api/pt-card/market/search`, results are client-filtered then merged & deduped (title-checked tags exclude untitled cards). Pills get a stable color per tag name; ✎ edits anytime; the binder magnifier links in (existing tag shows a pencil). While tags exist the market view switches to search results and hides the rarity/mech/title/price-threshold filters; clearing all tags returns to the regular market view
...
7. **Trade history / open orders / holdings / drop stats / card binder / portrait / market data**: sidebar entries fetch trades, orders and holdings (with stats panel and multi-dimension filters; sell prices shown as net after the 5% fee (sale price ÷ 1.05, hover for the original sale price); each holding has an **ask** button to query the top bid; trades sort by tradedAt, orders/holdings by last-modified time (lastModifiedDate), toggle asc/desc in the search bar); **Drop stats** triggers on view open: daily goes through `/pt-card/feed` direct (latest 25, after the latest msg); if >7 days since last refresh, opens a background tab for full re-collection (msg/search pagination — **considered complete once paging reaches past the drop-event origin (2026-07-01)**, no need to page through everything); if the tab can't get it all (e.g. browser not logged in to M-Team) the hero shows "only N records fetched" with a **manual import** (search "卡片掉落" on the message page yourself, copy the response JSON and paste it; multi-page import supported); views show rarity/title distribution, a daily drop bar chart (rarity-stacked, 7/30/90/All switchable) and activity (span / drop days / max streak / per-day avg), plus a **bonus-voucher usage** card (cards opened / total gain / average (buy price) / median (breakeven) / per-card high / low / lucky multiplier / volatility + gain-distribution bars + historical opening area-line chart); **Card binder** aggregates holdings + open listings by film (mech excluded): each film is an inverted-theme card with a 5-rarity × 5-title dual-cell matrix (left held green / right listed gold, dimmed levels exposing gaps at a glance), sorted by rarity variety → weighted rarity score → title score, with a magnifier button to add the film to directed search (shows a pencil if a tag exists; tap to edit its filters); **Portrait** collects Bonus daily (per-hour bonus income `finalBs`, shown at the profile card's magic corner like `94/h`) and synthesizes balance / income / trade cash flow / drop luck / voucher gambling into a wealth tier (Poor → Old Money, 9 levels), spending trend (Hoard / Self-made / Draining / Overdraw) and a 5-dimension radar (Wealth · Income · Spend · Luck · **Gamble** — derived from voucher-card openings (gambling intensity / luck / volatility), assigning a gamble persona None/Newbie/Lucky/Rollercoaster/Grind that feeds into the nickname); the luck-composition panel also includes a **voucher-gambling summary** (cards opened / total won / avg / best / worst / lucky multiplier); **Market data** aggregates full-site trade history (full pull after first token save, incremental on button click), showing market KPIs + time trends + card/people rankings, with a **one-click analysis report** (see "Market Data" section)
...
Manual buy calls `mtFetch` directly to `/api/pt-card/market/buy` (body: filmId/rarity/provenance/price), where **price = the tracked lowestAsk** (the card's current lowest ask at the moment you click buy). Before buying it only checks `tracked price ≤ set threshold` (hard cap) and the budget-pool balance; the buy flow runs under a **single-flight lock** (from confirm dialog through submission — no double-buy from double clicks).

**Own-listing guard**: a market card with the same filmId+rarity+provenance at the same price as one of your open sell orders is your own listing (the lowest ask is you) — its buy button is greyed out ("This is your own listing"), batch selection is blocked (click intercepted + checkbox hidden). No accidental self-buys.
...
- **Open order management**: each order in the Open orders view has a "Modify" button offering two choices — **cancel the order** (`/cancel`) or **modify the price** (no official reprice API: sell orders cancel then re-list via `sell`; **buy orders** cancel then re-list as a pure limit buy — unfilled stays as a buy order, fills instantly if a cheaper listing exists; the confirm button stays loading until the network op completes, Esc/cancel blocked meanwhile). The search bar has mutually exclusive Buy/Sell filter toggles (tap again to clear). Cancel/reprice runs under a single-flight lock, so double clicks never double-execute.
- **Locked cards**: trade locks (platform-side, temporary, auto-expiring) show "Cooling" (cooling cards **can be redeemed** — officially allowed); **manual locks** (your own, persistent) are **hidden by default** — check "Show Locked" in the filter bar to reveal them; unlocking takes a two-click confirmation.
...
- **Market · Batch buy**: select cards → budget pre-check right at the entry (no pool set / total exceeds usable → toast with the reason and amounts, no modal) → buy serially at each one's lowest ask, with total-cost-vs-usable-budget re-check before submit; limit-order semantics ensure no overpay, and unfilled (book moved) shows a grey "Unfilled" chip (excluded from retry); own listings can't be selected.
- **Holdings · Batch sell**: select cards → set net prices in bulk by rarity/title (or per row) → list serially. Adds new orders, so the 50-order cap is checked. Price inputs are **dual-state** — focused shows the editable net price, blurred shows the list price (net×1.05, dimmed, hover for both).
- **Normal-card redemption (10-for-1)**: the holdings view shows 3 recipe cards with redeemable counts (1=mana voucher N / 2=sticky free pass SR / 3=VIP pass UR; count = unlocked cards of that rarity ÷ 10) plus redemption history. Tap a card to enter **redeem mode** (list locked to that rarity, capped at exactly 10) — **auto-pick** takes the 10 cheapest by title tier (untitled → 傳火); a confirm modal lists all 10 cards with double validation front and back.
- **Sidebar change badges**: opening the panel probes each data source's total (5 tiny requests, nothing written) and shows red pill badges with new-item counts (99+ cap) on sidebar buttons; entering a view clears its badge.
- **Orders · Batch cancel**: select orders → cancel serially.
- **Orders · Batch reprice**: select orders → set a new net price → each order is cancelled then re-listed at the new price (sell: cancel + sell; buy: cancel + limit re-list).
...
**Market Analysis Report**: the "Generate Report" button at the end of the tab bar produces a print-grade HTML research report from the full history (**today excluded**, baseline = yesterday), opened in a new tab: ① volume×price quadrant diagnosis (expansion / distribution / divergence / cooling / flat) ② dominant structure (rarity GMV share + mech share + HHI concentration; regular and mech cards both count toward total GMV) ③ liquidity (net fund flow, pure-buyer/seller mix, flip margin & hold-day medians) ④ valuation sanity (skewness + rarity price ladder + dispersion + price-distribution histogram) ⑤ hot trades, with a one-line headline summary. Verdict tiers are language-neutral; the report renders in the UI language (EN/中文) and prints / saves to PDF directly.
...
- **Directed search**: tag list `{title, rarities[], titles[], per-rarity maxPrices{}}` (legacy plain-string tags = no filter; serial per-tag queries, client-side filtering, merged & deduped)
