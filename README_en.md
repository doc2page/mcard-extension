...
🎯 Directed search · 💰 Manual buy/sell/cancel · 📦 Batch buy/sell/cancel/reprice · 🚫 Own-listing guard · 💎 Bonus budget pool · 📊 Trades & orders · 📈 Drop stats (+📥 manual import backfill) · 💹 Market data (+📄 analysis report) · 📚 Card binder · 👤 Player portrait · 👁 Privacy mask · 🌍 Dual-site (.cc/.io) · 🎨 Dark/Light · 🌐 EN/中文
...
- **Open order management**: each order in the Open orders view has a "Modify" button offering two choices — **cancel the order** (`/cancel`) or **modify the price** (no official reprice API, so it cancels then re-lists at the new net price via `sell`); cancel/reprice runs under a single-flight lock, so double clicks never double-execute.
- **Locked cards**: trade locks (platform-side, temporary, auto-expiring) show "Cooling"; **manual locks** (your own, persistent) are **hidden by default** — check "Show Locked" in the filter bar to reveal them; unlocking takes a two-click confirmation.
